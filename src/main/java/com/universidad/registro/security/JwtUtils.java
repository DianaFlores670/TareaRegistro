...
public class JwtUtils {
    ...
    // Se inyecta desde el archivo de propiedades de la aplicación
    // (application.properties o application.yml)
    // Se espera que sea un número entero que representa la cantidad de milisegundos
    // antes de que el token expire
    // Se utiliza para establecer la fecha de expiración del JWT
    @Value("${app.jwtExpirationMs}")
    private int jwtExpirationMs;

    // Este método se utiliza para extraer el token JWT del encabezado de
    // autorización de la solicitud HTTP
    // El token JWT se espera que esté en el formato "Bearer <token>"
    // Si el encabezado de autorización no está presente o no tiene el formato
    // correcto, se devuelve null
    public String generateJwtToken(Authentication authentication) {
        UserDetails userPrincipal = (UserDetails) authentication.getPrincipal();

        // Se utiliza para generar un nuevo token JWT utilizando la información del
        // usuario autenticado
        // Se establece el sujeto del token como el nombre de usuario del usuario
        // autenticado
        return Jwts.builder()
                .setSubject((userPrincipal.getUsername()))
                .setIssuedAt(new Date())
                .setExpiration(new Date((new Date()).getTime() + jwtExpirationMs))
                .signWith(new javax.crypto.spec.SecretKeySpec(jwtSecret.getBytes(),
    ...
    public boolean validateJwtToken(String authToken) { // Este método se utiliza para validar el token JWT
        // Se utiliza para verificar la firma del token y asegurarse de que no haya sido
        // modificado
